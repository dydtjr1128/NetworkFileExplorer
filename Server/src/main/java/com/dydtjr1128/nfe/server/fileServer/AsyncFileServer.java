...
public class AsyncFileServer implements Runnable {
    ...
    private void handleNewConnection(AsynchronousSocketChannel channel) throws IOException, ExecutionException, InterruptedException {
        ...
        } else if (action == FileAction.FILE_SEND_TO_CLIENT) { // PATH + filename
            logger.debug("[File send to client]");
            Connection connection = clientManager.getClientByIP((InetSocketAddress) channel.getRemoteAddress());
            TransferFileMetaData metaData;
            if (connection == null) return;
            synchronized (connection.getFilePathQueue()) {
                Queue<TransferFileMetaData> queue = connection.getFilePathQueue();
                metaData = queue.poll();
            }
            if (metaData != null)
